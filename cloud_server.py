...
def call_groq(messages, tools=None):
    payload = {"model": MODEL_NAME, "messages": messages, "temperature": 0.6, "max_tokens": 700}
    if tools:
        payload["tools"] = tools
...
def ask_ai(user_prompt, extra_context=""):
    if not GROQ_API_KEY:
    ...
    msg = call_groq(messages, tools=TOOLS)
    ...
    if msg.get("tool_calls"):
        ...
        final = call_groq(messages)  # no tools needed on the follow-up
...
def upload():
    ...
    reply = ask_ai(prompt, extra_context=extra_context)
